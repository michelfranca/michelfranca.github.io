<div align="center">
  <h1>Jornada para Especialista em Dados & Business Intelligence</h1>
</div>

<p align="center">
  <img src="https://github.com/michelfranca/michelfranca.github.io/blob/main/Banner_GitHub.png?raw=true" width="800"/>
</p>

## Olá, eu sou Michel Oliveira França!

Profissional apaixonado por transformar dados em decisões. Meu foco é construir a ponte entre a análise técnica e a estratégia de negócio, dominando desde a modelagem de dados em SQL até a criação de visualizações e dashboards interativos em Power BI que apoiam o planejamento e a geração de valor.

--------

Este espaço é o meu portfólio digital e diário de bordo. Aqui você encontrará:
*   **Projetos Práticos:** Dashboards e análises que demonstram minhas habilidades.
*   **Roadmap de Estudos:** Meu plano de aprendizado detalhado e progresso atual.
*   **Artigos e Anotações:** Conteúdos que escrevo para solidificar meu conhecimento.

---

## Navegação Rápida

Explore as principais seções do meu portfólio:

| Seção | Descrição | Link |
| :--- | :--- | :--- |
| 🗺️ **Meu Roadmap** | A trilha de estudos detalhada que estou seguindo, com checklists de progresso. | [Acessar Roadmap](./ROADMAP.md ) |
| 📂 **Projetos de BI** | Uma galeria com meus principais dashboards e análises de dados. | [Ver Projetos](./PROJECTS.md) |
| 🐍 **Projetos de Python** | Scripts e notebooks de Python para análise e automação de dados. | [Ver Projetos](./PYTHON_PROJECTS.md) |
| ✍️ **Blog/Artigos** | Meus artigos sobre DAX, SQL, Modelagem de Dados e outros tópicos. | [Ler Artigos](./BLOG.md) |

---

## Minhas Principais Ferramentas

<p align="left">
  <a href="https://www.microsoft.com/pt-br/power-platform/products/power-bi" target="_blank" rel="noreferrer">
    <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black" alt="Power BI"/>
  </a>
  <a href="https://www.python.org" target="_blank" rel="noreferrer">
    <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python"/>
  </a>
  <a href="https://www.microsoft.com/pt-br/sql-server" target="_blank" rel="noreferrer">
    <img src="https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white" alt="SQL Server"/>
  </a>
  <a href="https://powerautomate.microsoft.com/pt-br/" target="_blank" rel="noreferrer">
    <img src="https://img.shields.io/badge/Power%20Automate-0066FF?style=for-the-badge&logo=powerautomate&logoColor=white" alt="Power Automate"/>
  </a>
</p>

---

## Conecte-se Comigo

Estou sempre aberto a novas conexões, oportunidades e discussões sobre dados!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white )](https://www.linkedin.com/in/michel-frança-346b24309 )
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white )](https://github.com/seu-github-aqui/ )
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white )](mailto:seu.email@gmail.com)

